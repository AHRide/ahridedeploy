...
const ModalReportV1 = ({ updateID, rider_email, client_email, from, to }) => {
	...
	useEffect(() => {
		const interval = setInterval(() => {
			axios
				.get(`https://ahride.herokuapp.com/getDeliveryUpdates/${updateID}/`)
				.then((response) => {
	...
	const saveUpdatedPost = (post) => {
		...
		axios.post('https://ahride.herokuapp.com/createClientReport', {
			deliveryID: updateID,
			client_email,
			rider_email,
			to,
		...
		axios
			.put(
				`https://ahride.herokuapp.com/getDeliveryUpdates/reported/${updateID}`,
				post
			)
