...
const StatusOfDelivery = () => {
	...
	useEffect(() => {
		axios
			.get(
				`https://ahride.herokuapp.com/getDeliveryUpdates/rider/${user._profile.data.email}`
			)
			.then((response) => {
	...
	const onSubmit = (
		...
		axios.post('https://ahride.herokuapp.com/deliveryHistory', {
			delivery_id: _id,
			status: 'Cancelled',
		...
		axios
			.delete(`https://ahride.herokuapp.com/getDeliveryUpdates/${_id}/`)
			.then((response) => {
	...
	const onPickingUpdate = (post) => {
		...
		axios
			.put(
				`https://ahride.herokuapp.com/getDeliveryUpdates/PickingUI/${post._id}/`,
				post
			)
		...
		axios
			.put(
				`https://ahride.herokuapp.com/getDeliveryUpdates/start/${post._id}/`,
				post
			)
			.then((res) => console.log(res))
			.catch((err) => console.log(err));
	...
	const onPickedUpdate = (post) => {
		...
		axios
			.put(
				`https://ahride.herokuapp.com/getDeliveryUpdates/PickedUI/${post._id}/`,
				post
			)
			.then((res) => console.log(res))
			.catch((err) => console.log(err));
	...
	const onOTWUpdate = (post) => {
		...
		axios
			.put(
				`https://ahride.herokuapp.com/getDeliveryUpdates/OTW/${post._id}/`,
				post
			)
			.then((res) => console.log(res))
			.catch((err) => console.log(err));
						...
						<Row>
							<Col xs={9} className='LeftPane'>
								<div class='d-flex justify-content-end' size='medium'>
									<ModalReportV1
										className='Rider_Report_Modal'
										updateID={lists._id}
										rider_email={user._profile.data.email}
										client_email={lists.client_email}
										from={lists.from}
										to={lists.to}></ModalReportV1>
								</div>
