...
const ClientUpdatesTab = () => {
	...
	useEffect(() => {
		const interval = setInterval(() => {
			if (location.state.pending === 'pending') {
				...
				axios
					.get(
						`https://ahride.herokuapp.com/getDeliveryUpdates/${location.state._id}/`
					)
					.then((response) => {
						setUpdateList(response.data);
					});
				...
				axios
					.get(
						`https://ahride.herokuapp.com/getUserRider/${location.state.email}`
					)
					.then((response) => {
	...
	const onSubmit = (_id) => {
		axios
			.delete(`https://ahride.herokuapp.com/getBookDelivery/${_id}/`)
			.then((response) => {
