...
export default function AdminClientInformation() {
	...
	useEffect(() => {
		axios
			.get(
				`https://ahride.herokuapp.com/getClientReports/${location.state._id}/`
			)
			.then((response) => {
		...
		axios
			.get(
				`https://ahride.herokuapp.com/getUser/${location.state.client_email}/`
			)
			.then((response) => {
