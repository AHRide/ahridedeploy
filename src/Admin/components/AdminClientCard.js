...
export default function AdminClientCard() {
	...
	useEffect(() => {
		const interval = setInterval(() => {
			axios
				.get(`https://ahride.herokuapp.com/getClientReports`)
				.then((response) => {
	...
	const toInfoStatus = (_id, client_email) => {
		navigate('/admin/client/report/information', {
			state: { _id, client_email },
		});
	};
					...
					<div
						...
						<CardContent>
							...
							<div className={style.stars}>
								<h3 style={{ marginLeft: '64rem' }}>Reported</h3>
							</div>
						</CardContent>
					</div>
				</div>
